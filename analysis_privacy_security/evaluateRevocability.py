...
def evaluate(matedScoresFile, nonMatedScoresFile, genuineScoresFile, figureFile):
	figureTitle = 'Revocability Analysis'
	...
	ax = sns.kdeplot(matedScores, fill=False, label='Mated-imposter', color=sns.xkcd_rgb["medium green"])
	x1,y1 = ax.get_lines()[0].get_data()
	ax = sns.kdeplot(nonMatedScores, fill=False, label='Imposter', linewidth=5, color=sns.xkcd_rgb["pale red"], linestyle='--')
	x2,y2 = ax.get_lines()[1].get_data()
	ax = sns.kdeplot(genuineScores, fill=False, label='Genuine', linewidth=5, color=sns.xkcd_rgb["medium blue"])
	...
	# ax.set_title("Revocability Analysis")
